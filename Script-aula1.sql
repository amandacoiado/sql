...
SELECT
    cliente_id,
    TRANSLATE(nome, 'áàâãéèêíìîóòôõúùûçÁÀÂÃÉÈÊÍÌÎÓÒÕÚÙÛÇ', 'aaaaeeeeiiioooouuucAAAAEEEIIIOOOUUUC') AS nome_sem_acento,
    endereco,
    cidade_id
FROM Clientes
...
SELECT * FROM `TiposTransacao` WHERE lower(`nome_tipo_transacao`) like 'depósito%';
...
/* 3) Qual o dia do ano maior quantidade de transações em 2024? */
...
/* --------------------------------------------------------
 * 5ª parte - Junção de Tabelas (JOIN) 
 * --------------------------------------------------------*/
...
FROM `Agencias`a
	FULL OUTER JOIN `Cidades`b
		ON a.`cidade_id` = b.`cidade_id`;
-- Registros: 184
-- Corresponde ao maior número de junções possíveis entre as tabelas
...
-- Qual a Cidade de localização da Agência com maior número de contas ativas?

-- Qual o tipo de transação com maior número de registros?

-- Qual o o tipo de local com maior número de registros?

-- Qual o cliente com maior número de transações (tanto de destino quanto de origem)?
